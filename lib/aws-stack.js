...
function logOperation(operation, stackConfig, params){
    ...
    if( params ){
        ...
        console.info(`\tTemplate Uri: ${stackConfig.stackTemplateS3Uri}/templates/template.${stackConfig.stackTemplateFormat}`);
    }
    console.info(``);
}

function getTemplateFormat(templateLocation){
    return Fs.readdirSync(templateLocation)
...
function validateStackConfig(stackConfig, validate){
    ...
    stackConfig.stackTemplateType   = stackConfig.stackTemplateType || stackConfig.stackEnv === "static" ? "static" : "dynamic";
    ...
    stackConfig.stackTemplateFormat = getTemplateFormat(stackConfig.stackTemplatePath) || 'json';
...
function fromS3UriToS3Url(stackTemplateS3Uri, stackRegion){
    return stackTemplateS3Uri.replace("s3://",`https://s3-eu-west-1.amazonaws.com/`)
}
...
class AwsStack {
    ...
    constructor(stackConfig, validate = true){
        ...
        // this._awsS3  = S3.createClient({s3Client: new AWS.S3()});
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            let interval = setInterval(() => {
                this._awsCf.describeStacks(request, (error, response) => {
                    ...
                    if(response.Stacks[0].StackStatus.endsWith('FAILED')
                       || response.Stacks[0].StackStatus.includes('FAILED')
                       || response.Stacks[0].StackStatus.startsWith('ROLLBACK')
                       || response.Stacks[0].StackStatus.contains('UPDATE_ROLLBACK_IN_PROGRESS')){
                        didDeploymentFailed = true;
                    }
                    console.info(`   ${response.Stacks[0].StackStatus}`);
                    if( response.Stacks[0].StackStatus.endsWith('COMPLETE') ){
                        clearInterval(interval);
                        return didDeploymentFailed ? reject("Failed Deployment") : resolve(response.Stacks[0])
                    }
                });
            }, 10000);
        });
    ...
    create(){
        ...
        return new Promise((resolve, reject) => {
            let request = {
                ...
                TemplateURL : fromS3UriToS3Url(`${this._stackConfig.stackTemplateS3Uri}/templates/template.${this._stackConfig.stackTemplateFormat}`, this._stackConfig.stackRegion)
            };
    ...
    publish(){
        ...
        return new Promise((resolve, reject) => {
            Dir(this._stackConfig.stackPath, (error, paths) => {
                if( error ){
                    return reject(error);
                }
                let result = paths
                    .map( path => ([path, path.replace(`${this._stackConfig.stackPath}/`, "")]))
                    .map( ([path, file]) => new Promise((resolve,reject) => {
                    let request = {
                        Bucket: s3Bucket,
                        Key   : `${s3Key}/${file}`,
                    ...
                    this._awsS3.upload(request, (error) => {
                        error ? reject(error) : resolve();
                    });
                }));
                Promise.all(result).then( () => resolve(), (error) => reject(error));
            });
        });
    }
}
